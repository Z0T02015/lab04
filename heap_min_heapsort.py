class Heap:
    ...
    def percolateDown(self, i):
        pass  # será implementado por MinHeap

class MinHeap(Heap):
    def percolateDown(self, i):
        while self.leftChildIndex(i) < self.size:
            minIdx = self.minimumChildIndex(i)
            if self.heapList[i] > self.heapList[minIdx]:
                self.heapList[i], self.heapList[minIdx] = self.heapList[minIdx], self.heapList[i]
            i = minIdx

# ---------------------- PRUEBA Heapsort con MinHeap ----------------------
